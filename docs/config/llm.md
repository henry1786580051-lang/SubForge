...
LLM（大语言模型）是 SubForge 的核心功能之一，用于字幕断句、优化和翻译。本指南将帮助你配置 LLM API。
...
SubForge 支持多种 LLM 服务商，你可以根据自己的需求选择：
...
1. **注册账号**
   ...
   访问 [https://api.subforge.cn/register](https://api.subforge.cn/register?aff=UrLB) 注册（通过链接注册赠送 $0.4 测试余额）

2. **获取 API Key**

   登录后访问 [Token 页面](https://api.subforge.cn/token) 获取 API Key

3. **在 SubForge 中配置**
   - **LLM 服务**: 选择 `OpenAI`（兼容模式）
   - **API Base URL**: `https://api.subforge.cn/v1`
