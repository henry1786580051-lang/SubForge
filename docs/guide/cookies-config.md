...
:::tip 何时需要配置
只有当你看到上述错误提示时才需要配置 Cookie。大多数情况下，SubForge 可以直接下载视频。
:::
...
将导出的 `cookies.txt` 文件移动到 SubForge 的 **AppData** 目录下。

#### AppData 目录位置

SubForge 的 AppData 目录通常位于：
...
:::tip 快速定位
在 SubForge 中点击 **设置 → 打开日志文件夹**，然后返回上一级目录即可看到 `AppData` 文件夹。
:::
...
SubForge 使用 [yt-dlp](https://github.com/yt-dlp/yt-dlp) 作为下载引擎，支持 1000+ 个视频网站，包括：
