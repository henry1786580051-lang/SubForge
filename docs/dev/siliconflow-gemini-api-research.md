...
This note records the current API surface and local connectivity tests for adding SiliconFlow and Gemini text/TTS providers to SubForge. API keys used during testing are intentionally omitted.
...
SubForge already has a partial implementation in `subforge/core/tts/siliconflow.py`:
...
Gemini is not directly compatible with the current `OpenAI` client path used by `subforge/core/llm/client.py`. It needs either:

1. a Gemini-native LLM client using `generateContent`, or
2. a provider adapter that maps SubForge messages/config into Gemini REST calls.
