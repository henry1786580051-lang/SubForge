...
以「2026 Lexus ES 350h 试驾」视频（21 分钟英文）为例，使用小米 MiMo v2.5-pro 完成全流程（分句 + 优化 + 翻译）：
...
> MiMo v2.5-pro 是推理模型，Reasoning tokens 占 Completion 的 86%（126K / 146K），实际输出内容约 20K tokens。

关键指标：
- **单视频总消耗**：~200K tokens，其中推理开销占大头
- **优化阶段最耗资源**：占总 token 的 50%，因反馈循环（相似度不达标时自动重试）
- **翻译阶段最高效**：20 次调用仅 30K tokens，5.6 分钟完成
- **最终产出**：389 段双语字幕（中文 + 英文），覆盖 21 分钟视频

> 使用免费翻译（Bing / Google）可跳过 LLM 翻译阶段，token 消耗降低约 15%。使用本地 LM Studio 等免费模型可完全零成本。
