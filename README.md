...
SubForge 的核心优势在于 **LLM 驱动的智能断句** 和 **上下文感知翻译**，而非简单的逐句机翻。

### 智能断句

语音识别输出的原始文本通常是连续的长句，SubForge 使用 LLM 按语义自然断点重新分段，让字幕更易阅读：
...
> 示例文件：[`examples/sample_en.srt`](examples/sample_en.srt)（原文）→ [`examples/sample_en_processed.srt`](examples/sample_en_processed.srt)（处理后）

### 智能翻译

与逐句机翻不同，SubForge 的 LLM 翻译会结合上下文理解整段内容，输出自然流畅的目标语言：
...
> 示例文件：[`examples/ru_original.srt`](examples/ru_original.srt)（原文）→ [`examples/ru_processed.srt`](examples/ru_processed.srt)（翻译后）

### 处理流程

```
音视频输入 → 语音识别 → LLM 智能断句 → LLM 纠错优化 → 上下文翻译 → 双语字幕输出
```

- **智能断句**：按语义自然分段，不截断完整语义，中英文分别限制字数
- **纠错优化**：去除语气词（um、uh、呃），修正错别字，规范标点格式
- **上下文翻译**：理解整段语境，使用地道表达，而非逐词直译
- **反思机制**：支持多轮优化，提升字幕准确率
