...
allowed-tools: Bash(subforge *, ffprobe *, ffmpeg -ss *)
---
...
**Always run `subforge <command> --help` first** to check the latest options and defaults before executing a command. The examples below are common patterns, but --help is the source of truth.

- Install: `pip install subforge`
...
```bash
subforge process video.mp4 --asr bijian --translator bing --target-language en \
...
```bash
subforge transcribe video.mp4 --asr bijian -o output.srt

# Output as JSON format to a directory
subforge transcribe video.mp4 --asr bijian --format json -o ./subtitles/
```
...
subforge subtitle input.srt --translator bing --target-language en --layout target-above -o translated.srt

# Free Google → Japanese, translation only (discard original text)
subforge subtitle input.srt --translator google --target-language ja --no-optimize --layout target-only -o output_ja.srt

# High quality LLM translation with reflective mode
subforge subtitle input.srt --translator llm --target-language en --reflect \
...
subforge process video.mp4 --asr bijian --translator google --target-language ko \
...
```bash
# Step 1: Transcribe
subforge transcribe video.mp4 --asr bijian -o video.srt

# Step 2: Translate (bilingual, original text above translation)
subforge subtitle video.srt --translator bing --target-language en --layout source-above -o video_en.srt

# Step 3: Burn into video with rounded background, high quality
subforge synthesize video.mp4 -s video_en.srt --subtitle-mode hard \
...
```bash
subforge process podcast.mp3 --asr bijian --translator bing --target-language en -o ./output/
```

### 8. Transcribe other languages (whisper-api)

```bash
subforge transcribe french_video.mp4 --asr whisper-api \
...
```bash
subforge subtitle raw_subtitle.srt --no-translate --api-key $OPENAI_API_KEY -o optimized.srt
```
...
Run `subforge <command> --help` for full options.
...
- Run `subforge style` to see all style presets
