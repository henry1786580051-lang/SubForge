...
Examples:
    python scripts/translate_llm.py resource/translations/SubForge_en_US.ts
    python scripts/translate_llm.py resource/translations/SubForge_zh_HK.ts
    python scripts/translate_llm.py resource/translations/SubForge_ja_JP.ts
"""
import os
import re
import sys
...
def detect_target_language(filename: str) -> str:
    ...
    # Extract locale code (e.g., "en_US" from "SubForge_en_US.ts")
    match = re.search(r"_([a-z]{2}_[A-Z]{2})\.ts$", filename)

    if not match:
        raise ValueError(
            f"Cannot detect language from filename: {filename}\n"
            f"Expected format: SubForge_<locale>.ts (e.g., SubForge_en_US.ts)"
        )
