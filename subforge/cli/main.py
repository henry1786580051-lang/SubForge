"""SubForge CLI — AI-powered video captioning from the command line.

Usage:
    subforge <command> [options]
...
def _add_style_options(parser: argparse.ArgumentParser) -> None:
    ...
    grp = parser.add_argument_group(
        ...
        description="Style options only take effect with hard subtitles. "
                    ...
                    "Use 'subforge style' to see available presets.",
    )
    grp.add_argument(
        "--render-mode",
        choices=["ass", "rounded"],
...
def _build_gui_parser(subparsers) -> None:
    p = subparsers.add_parser(
        ...
        description="Launch the SubForge desktop app.",
    )
...
def _build_subtitle_parser(subparsers) -> None:
    ...
    sub.add_argument("--max-cjk", type=int, metavar="N", help="Hard character limit for CJK subtitles (default: 25)")
    sub.add_argument("--max-english", type=int, metavar="N", help="Target word count for English subtitles (default: 18; hard limit: target + 4)")
...
def _build_dub_parser(subparsers) -> None:
    from subforge.core.dubbing.presets import available_dubbing_presets
...
def _build_config_parser(subparsers) -> None:
    p = subparsers.add_parser(
        ...
        description="View, edit, and manage SubForge configuration.",
    )
    ...
    init_p = config_sub.add_parser(
        ...
        description=(
            "Create a SubForge config file. By default this starts an interactive setup. "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog="Run 'subforge <command> --help' for details on each command.",
    )
...
def _run_gui(_args: argparse.Namespace) -> int:
    try:
        from subforge.ui.main import main as gui_main
    except ImportError as exc:
        print(f"GUI dependencies are not available: {exc}")
        print("Install the official package with: pip install subforge")
...
def _run_synthesize(args: argparse.Namespace) -> int:
    from subforge.cli.commands.synthesize import run
    config = _load_config(args)
    return run(args, config)


def _run_dub(args: argparse.Namespace) -> int:
    from subforge.cli.commands.dub import run
    config = _load_config(args)
    return run(args, config)
...
def _run_style(args: argparse.Namespace) -> int:
    from subforge.cli.commands.style_cmd import run
    config = _load_config(args)
    return run(args, config)
