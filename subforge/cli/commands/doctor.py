...
from subforge.cli.config import CONFIG_FILE, DEFAULTS, get
from subforge.core.dubbing.presets import (
    get_dubbing_preset,
...
def _check_dubbing(config: dict) -> list[Check]:
    ...
    if preset_name:
        try:
            ...
            checks.append(Check("dubbing.preset", "error", str(exc), "Choose one of the presets shown in 'subforge dub --help'"))
    else:
        checks.append(Check("dubbing.preset", "warn", "No dubbing preset configured", "Run 'subforge config set dubbing.preset edge-cn-female'"))
    if provider != "edge" and not get(config, "dubbing.api_key", ""):
        checks.append(Check("dubbing.api_key", "warn", "Dubbing TTS API key is missing", "Run 'subforge config set dubbing.api_key <key>'"))
...
def _check_api(config: dict) -> list[Check]:
    ...
    if not get(config, "dubbing.api_key", ""):
        return checks
    checks.append(Check("api.dubbing", "warn", "--check-api is currently limited to configuration validation", "Run a short 'subforge dub sample.srt' to verify billing/provider access"))
    return checks
