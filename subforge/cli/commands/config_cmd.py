...
from subforge.cli.config import (
    CONFIG_FILE,
    DEFAULTS,
    _set_nested,
    _write_toml,
...
def run(args: Namespace, config: dict) -> int:
    ...
    if action == "show":
        ...
        print("Usage: subforge config <show|set|get|path|init|edit>")
...
def _path() -> int:
    ...
    if not exists:
        output.hint("File does not exist yet. Run 'subforge config init' to create it.")
    return EXIT.SUCCESS
...
def _set(key: str, value: str) -> int:
    ...
    if default_val is None:
        ...
        output.hint("Run 'subforge config show' to see available keys.")
...
def _interactive_init(args: Namespace, config_data: dict) -> int:
    ...
    if CONFIG_FILE.exists() and not getattr(args, "force", False):
        ...
        output.hint("Use 'subforge config init --force' to overwrite it.")
        output.hint("Use 'subforge config edit' to modify the existing file.")
        return EXIT.USAGE_ERROR
    ...
    def _prompt(msg: str, default: str = "") -> str:
        try:
            ...
            print()
            output.hint("Non-interactive mode detected. Use 'subforge config init --non-interactive'.")
            raise

    print("SubForge Onboarding")
    print("=" * 40)
    print()
...
def _write_onboarding_config(template: str, *, force: bool) -> int:
    ...
    if CONFIG_FILE.exists() and not force:
        output.warn(f"Config file already exists: {CONFIG_FILE}")
        output.hint("Use --force to overwrite, or 'subforge config edit' to modify it.")
        return EXIT.USAGE_ERROR
    ...
    output.hint("Run 'subforge doctor' to check dependencies and missing keys.")
    return EXIT.SUCCESS
...
def _edit() -> int:
    ...
    if not CONFIG_FILE.exists():
        ...
        from subforge.cli.config import _write_toml
