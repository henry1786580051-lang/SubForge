...
from subforge.core.subtitle.style_manager import StyleMode, list_styles
...
def run(args: Namespace, config: dict) -> int:
    ...
    if not action:
        output.error("No action specified. Use: subforge style")
...
def _list(args: Namespace) -> int:
    ...
    print("\nUsage:")
    print("  subforge synthesize video.mp4 -s sub.srt --subtitle-mode hard --style <name>")
    print("  subforge synthesize video.mp4 -s sub.srt --subtitle-mode hard --style-override '{\"outline_color\": \"#ff0000\"}'")

    print("  subforge synthesize video.mp4 -s sub.srt --subtitle-mode hard --render-mode rounded")
